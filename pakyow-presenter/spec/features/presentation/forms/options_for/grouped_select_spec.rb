...
RSpec.describe "populating groups of options" do
  ...
  it "creates a group for each group" do
    expect(tag_view.delegate.find_significant_nodes(:optgroup, tag_view.object).count).to eq(2)
  end

  it "sets the label for each optgroup" do
    groups = tag_view.delegate.find_significant_nodes(:optgroup, tag_view.object)
  ...
  it "creates an option for each value" do
    groups = tag_view.delegate.find_significant_nodes(:optgroup, tag_view.object)
    expect(tag_view.delegate.find_significant_nodes(:option, groups[0]).count).to eq(3)
    expect(tag_view.delegate.find_significant_nodes(:option, groups[1]).count).to eq(1)
  end

  it "sets the submitted value for each option" do
    groups = tag_view.delegate.find_significant_nodes(:optgroup, tag_view.object)

    group1_options = tag_view.delegate.find_significant_nodes(:option, groups[0])
    ...
    group2_options = tag_view.delegate.find_significant_nodes(:option, groups[1])
  ...
  it "sets the presentation value for each option" do
    ...
    group1_options = tag_view.delegate.find_significant_nodes(:option, groups[0])
    expect(tag_view.delegate.node_text(group1_options[0])).to eq("1.1")
    expect(tag_view.delegate.node_text(group1_options[1])).to eq("1.2")
    expect(tag_view.delegate.node_text(group1_options[2])).to eq("1.3")

    group2_options = tag_view.delegate.find_significant_nodes(:option, groups[1])
    expect(tag_view.delegate.node_text(group2_options[0])).to eq("2.1")
  end
end
